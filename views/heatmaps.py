...
page_header(
    eyebrow="Analiza przestrzenna",
    title="Mapy cieplne",
    subtitle="Strefy aktywności na podstawie playermatchstats.csv oraz — jeśli dostępne — rzeczywiste mapy zdarzeń z events.csv.",
)

ZONE_METRIC_FAMILIES = {
    "Dotknięcia ofensywne": ("OFFENSIVE_TOUCHES_IN_PITCH_POSITION_{z}", "OFFENSIVE_TOUCHES_IN_LANE_{l}", COLORS["accent"]),
    "Ominięci rywale (packing)": ("BYPASSED_OPPONENTS_FROM_PITCH_POSITION_{z}", "BYPASSED_OPPONENTS_FROM_LANE_{l}", COLORS["accent_3"]),
    "Odzyskania piłki": ("BALL_WIN_NUMBER_FROM_PITCH_POSITION_{z}", "BALL_WIN_NUMBER_IN_LANE_{l}", COLORS["accent_4"]),
}
available_families = {
...
section_divider("Mapa strefowa (playermatchstats.csv)")
c1, c2, c3 = st.columns([1, 1, 1.4])
with c1:
    scope = st.radio("Zakres", ["Zawodnik", "Drużyna"], horizontal=True, key="hm_scope")
with c2:
    if scope == "Zawodnik":
        teams_opt = ["Wszystkie drużyny"] + get_teams(pms)
        team_for_players = st.selectbox("Filtruj wg drużyny", teams_opt, key="hm_team_filter")
        target_players = get_players(pms, team_for_players)
        target = st.selectbox("Zawodnik", target_players, key="hm_player")
    else:
        target = st.selectbox("Drużyna", get_teams(pms), key="hm_team")
with c3:
    metric_family = st.selectbox("Metryka", list(available_families.keys()) or ["Brak dostępnych metryk"], key="hm_metric")

if available_families and metric_family in available_families:
    pitch_tpl, lane_tpl, color = available_families[metric_family]
    subset = pms[pms["playerName"] == target] if scope == "Zawodnik" else pms[pms["squadName"] == target]
    ...
    st.caption(
        "Siatka to iloczyn dwóch niezależnych rozkładów brzegowych (strefa boiska × korytarz), "
        "bo dane raportują je osobno, a nie jako jedną wspólną tabelę — czyli **przybliżenie**, "
        "nie dokładny rozkład wspólny dotknięć na boisku."
    )
else:
    st.info("W załadowanym pliku playermatchstats.csv brak kompletu kolumn strefowych dla żadnej z obsługiwanych metryk.")

# ---------------------------------------------------------------- Zdarzenia (prawdziwe x/y)
events = load_events()
section_divider("Mapa zdarzeń (events.csv)")

if events.empty:
    st.info("Brak pliku `data/events.csv` — ta sekcja pojawi się automatycznie, gdy plik będzie dostępny w folderze data/.")
else:
    ...
    if not x_col or not y_col:
        ...
        with e1:
            ev_scope = st.radio("Zakres", ["Zawodnik", "Drużyna", "Wszyscy"], horizontal=True, key="ev_scope")
        name_col, squad_col = colmap.get("playerName"), colmap.get("squadName")
        ev_target = None
        with e2:
            if ev_scope == "Zawodnik" and name_col:
                ev_target = st.selectbox("Zawodnik", sorted(events[name_col].dropna().unique()), key="ev_player")
            elif ev_scope == "Drużyna" and squad_col:
                ev_target = st.selectbox("Drużyna", sorted(events[squad_col].dropna().unique()), key="ev_team")
        type_col = colmap.get("eventType")
        with e3:
            if type_col:
                types = sorted(events[type_col].dropna().unique())
                chosen_types = st.multiselect("Typy zdarzeń", types, default=types, key="ev_types")
            else:
                chosen_types = None
        ...
        if sub.empty:
            ...
            label = ev_target if ev_target else "wszystkich zdarzeń"
            fig = event_heatmap_figure(x_norm, y_norm, title=f"Mapa zdarzeń — {label}", height=480)
            st.plotly_chart(fig, width='stretch', config={"displayModeBar": False})
            st.caption(f"Mapa zbudowana z {len(sub)} zdarzeń o rzeczywistych współrzędnych (kolumny `{x_col}` / `{y_col}`).")

            shot_mask = sub[type_col].astype(str).str.contains("shot", case=False, na=False) if type_col else pd.Series(False, index=sub.index)
            if shot_mask.any():
                section_divider("Mapa strzałów")
                ...
                fig2 = shot_map_figure(sx, sy, xg_vals, outcome_flag, height=460, title=f"Strzały — {label}")
                st.plotly_chart(fig2, width='stretch', config={"displayModeBar": False})
                st.caption(
                    "Rozmiar znacznika ∝ xG strzału. Kolor oznacza wynik akcji wg kolumny outcome "
                    "(zaznaczone „udana” względem pozostałych), a nie potwierdzoną bramkę."
                )
